  ...
  : Node(name) 
{
  ...
  RCLCPP_INFO_STREAM(get_logger(), "The conversion matrix is " << std::endl << speed_conversion_);
...
int main(int argc, char* argv[]){
  ...
  auto node = std::make_shared<SimpleController>("simple_velocity_controller");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0; 
}